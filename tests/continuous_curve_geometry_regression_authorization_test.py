#!/usr/bin/env python3
"""Focused contract for generic repository-resident CGR execution authorization."""
...
def load_module(path: pathlib.Path):
    spec = importlib.util.spec_from_file_location("cgr_authorization", path)
...
def audit_value(value: dict) -> dict:
    return {
        "schema_version": 1,
        "kind": "continuous-curve-geometry-regression-preparation-audit",
        "decision": "PASS",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="apmesh-cgr-authorization-") as temporary:
        ...
        value = {
            ...
            "preparation_audit": "docs/audits/2026-09-21-continuous-curve-geometry-regression-preparation-audit.json",
        ...
        authorization = (
            authorization_dir
            / f"continuous-curve-geometry-regression-{manifest}.json"
        )
        ...
        require(
            ...
            == f"experiments/authorizations/continuous-curve-geometry-regression-{manifest}.json",
        ...
        require(rows["prepared_manifest_sha256"] == manifest, "manifest output differs")
        require(
            rows["preparation_seal_sha256"] == value["preparation_seal_sha256"],
            "seal output differs",
        )
        require(
            rows["claim_tag"] == f"cgr-execution-claim-{manifest}",
        ...
        wrong_name = authorization_dir / "continuous-curve-geometry-regression-wrong.json"

#!/usr/bin/env python3
"""Prepare and validate immutable GPR0-GPR7 manifests; never execute them."""
...
from experiment_runtime import RuntimeErrorEvidence, clean_candidate, input_identity, read_json, sha256_file, tool_version, utc_now, verify_input_identity, write_json, write_state
from geometry_primitives_cumulative_evidence import EvidenceError, validate_profile
...
PREPARATION_FILES = ("profile.json", "prepared-manifest.json", "plan.json", "planned-inventories.json", "state.json", "state-history.jsonl")
...
def protocol_check(path: pathlib.Path) -> None:
    ...
    required = ("## 9. GPR0-GPR7 gate", "execution_requested=false", "detached-worktree verification", "four-cell campaign remains unauthorized")
...
def preparation_seal(output: pathlib.Path) -> dict[str, Any]:
    history = (output / "state-history.jsonl").read_bytes()
    return {"schema_version": 1, "files": {name: sha256_file(output / name) for name in PREPARATION_FILES}, "state_history_sha256": hashlib.sha256(history).hexdigest()}
...
def validate_prepared(output: pathlib.Path, *, require_unconsumed: bool = True) -> dict[str, Any]:
    ...
    if set(seal) != {"schema_version", "files", "state_history_sha256"} or seal["schema_version"] != 1 or seal["files"] != {name: sha256_file(output / name) for name in PREPARATION_FILES} or seal["state_history_sha256"] != hashlib.sha256((output / "state-history.jsonl").read_bytes()).hexdigest():
    ...
    if len(history) != 1 or state != json.loads(history[0]) or state.get("state") != "PREPARED":
        raise fail("prepared lifecycle differs")
    if require_unconsumed and any((output / name).exists() for name in ("execution-claim.json", "terminal-manifest.json", "command-records.json")):

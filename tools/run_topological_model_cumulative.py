#!/usr/bin/env python3
"""Prepare, execute once, and retain immutable TMR0-TMR7 evidence."""
...
from topological_model_cumulative_evidence import (EvidenceError, compare,
...
def validate_prepared(output: pathlib.Path, *, require_unconsumed: bool = True) -> dict[str, Any]:
    ...
    plan = {"schema_version": 1, "kind": "topological-model-cumulative-launch-plan", "execution_requested": False, "cells": manifest["plan"], "gates": manifest["gates"], "limitations": manifest["retained_limitations"]}
...
def write_records(output: pathlib.Path, records: list[dict[str, Any]]) -> None:
    write_json(output / "command-records.json", {"schema_version": 1, "kind": "topological-model-cumulative-command-records", "records": records})


def write_terminal(output: pathlib.Path, manifest: dict[str, Any], state: str, extra: dict[str, str]) -> None:
    write_json(output / "terminal-manifest.json", {"schema_version": 1, "kind": "topological-model-cumulative-terminal-manifest", "candidate": manifest["candidate"], "prepared_manifest_sha256": sha256_file(output / "prepared-manifest.json"), "state": state, "command_records": "command-records.json", "retention_manifest": "retention-manifest.json", **extra})


def verify_detached_candidate(source: pathlib.Path, candidate: dict[str, Any]) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="apmesh-core-tmr-detached-") as temporary:
...
def write_retention_inventory(output: pathlib.Path, manifest: dict[str, Any], required_paths: set[str]) -> None:
    ...
    write_json(output / "retention-manifest.json", {"schema_version": 1, "kind": "topological-model-cumulative-retention", "candidate_commit": manifest["candidate"]["commit"], "prepared_manifest_sha256": sha256_file(output / "prepared-manifest.json"), "required_paths": sorted(required_paths), "files": files})
...
def verify_retention(output: pathlib.Path) -> dict[str, Any]:
    ...
    if set(retention) != {"schema_version", "kind", "candidate_commit", "prepared_manifest_sha256", "required_paths", "files"} or retention.get("schema_version") != 1 or retention.get("kind") != "topological-model-cumulative-retention" or retention.get("candidate_commit") != manifest["candidate"]["commit"] or retention.get("prepared_manifest_sha256") != sha256_file(output / "prepared-manifest.json") or set(retention.get("required_paths", [])) != expected:
    ...
    if detached != {"schema_version": 1, "kind": "topological-model-cumulative-detached-verification", "result": "PASS", "candidate_commit": manifest["candidate"]["commit"], "source_inventory_count": len(manifest["candidate"]["source_inventory"])}:
        raise fail("detached verification differs")
    return {"schema_version": 1, "kind": "topological-model-cumulative-retention-verification", "status": "PASS", "files": sorted(listed)}
...
def seal_output(output: pathlib.Path, manifest: dict[str, Any], required_paths: set[str]) -> None:
    ...
    write_json(output / "detached-verification.json", {"schema_version": 1, "kind": "topological-model-cumulative-detached-verification", **detached})
...
def per_cell_comparisons(profile: dict[str, Any], entries: list[dict[str, Any]], output: pathlib.Path) -> dict[str, Any]:
    ...
    return {"schema_version": 1, "kind": "topological-model-cumulative-per-cell-comparisons", "cells": cells}


def gate_summary(profile: dict[str, Any], limitations: list[str]) -> dict[str, Any]:
    return {"schema_version": 1, "kind": "topological-model-cumulative-gate-summary", "status": "EVIDENCE_COLLECTED_PENDING_AUDIT", "gates": {gate: "EVIDENCE_COLLECTED_PENDING_AUDIT" for gate in profile["gates"]}, "limitations": limitations}
...
def execute(arguments: argparse.Namespace) -> int:
    ...
    try:
        ...
        write_json(output / "execution-claim.json", {"schema_version": 1, "kind": "topological-model-cumulative-execution-claim", "candidate_commit": manifest["candidate"]["commit"], "prepared_manifest_sha256": sha256_file(output / "prepared-manifest.json"), "pid": os.getpid(), "started_utc": utc_now()})
        ...
        write_json(output / "observed-inventories.json", {"schema_version": 1, "kind": "topological-model-cumulative-observed-inventories", "candidate_commit": manifest["candidate"]["commit"], "discoveries": discoveries, "cells": inventories})
        summary = gate_summary(profile, manifest["retained_limitations"])
        write_json(output / "gate-summary.json", summary)
        (output / "gate-summary.md").write_text("# TMR0-TMR7 execution evidence\n\nStatus: EVIDENCE_COLLECTED_PENDING_AUDIT\n", encoding="utf-8")
        ...
        write_records(output, records)
        manifest = read_json(output / "prepared-manifest.json")
        write_json(output / "failure.json", {"schema_version": 1, "kind": "topological-model-cumulative-failure", "message": str(error), "records": records, "partial_certificate_slots": entries})
...
def self_check(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    plan = command_plan(profile, source, declared)
    if len(plan) != 4 or any(cell["repetitions"] != 2 for cell in plan):
        raise fail("fixed matrix differs")
    return {"schema_version": 1, "kind": "topological-model-cumulative-runner-self-check", "status": "PASS", "execution_requested": False, "input_hashes": input_identity(paths), "declared_semantic_ctest_allowlist": declared, "plan": {"schema_version": 1, "kind": "topological-model-cumulative-launch-plan", "execution_requested": False, "cells": plan, "gates": {gate: "NOT_EXECUTED" for gate in profile["gates"]}, "limitations": profile["limitations"]}}

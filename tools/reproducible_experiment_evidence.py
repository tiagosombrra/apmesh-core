...
def _validate_execution(root: pathlib.Path) -> dict[str, Any]:
    ...
    record_keys = {"schema_version", "kind", "id", "argv", "cwd", "environment_delta", "started_utc", "ended_utc", "elapsed_seconds", "pid", "timeout_seconds", "timed_out", "exit_code", "stdout", "stderr"}
...
def _inventory_entries(root: pathlib.Path, profile: dict[str, Any], execution: dict[str, Any]) -> list[tuple[str, str, dict[str, Any] | None]]:
    ...
    for path in profile["required_artifacts"]:
        ...
        entries.append((path, path, derivation))
    for record in execution["records"]:
        entries.extend((record[stream]["path"], f"command-{stream}", None) for stream in ("stdout", "stderr"))
    return entries
...
def validate_bundle(root: pathlib.Path, profile: dict[str, Any]) -> dict[str, Any]:
    ...
    if inventory["schema_version"] != 1 or inventory["kind"] != "experiment-artifact-inventory" or inventory["artifacts"] != expected:
...
def compare_configurations(cell_summaries: dict[str, dict[str, Any]], profile: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"baseline": baseline_name, "cells": list(EXPECTED_CONFIGURATION_NAMES), "claim_fields": "equivalent", "differences": []}
